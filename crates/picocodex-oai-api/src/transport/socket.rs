...
impl ResponsesSocket {
    ...
    pub(crate) async fn connect(
        ...
        request.headers_mut().insert(
            header::USER_AGENT,
            HeaderValue::from_static(concat!("picocodex/", env!("CARGO_PKG_VERSION"))),
        );
...
mod tests {
    ...
    async fn respects_http_proxy_for_websocket_connections() -> Result<()> {
        run_proxy_test(
            "HTTP_PROXY",
            "ws://unreachable.picocodex.invalid/v1/responses",
            "unreachable.picocodex.invalid:80",
            None,
        )
        .await
    }
    ...
    async fn respects_https_proxy_for_secure_websocket_connections() -> Result<()> {
        run_proxy_test(
            "HTTPS_PROXY",
            "wss://unreachable.picocodex.invalid/v1/responses",
            "unreachable.picocodex.invalid:443",
            Some(502),
        )
        .await
    }
    ...
    async fn run_proxy_test(
        ...
        command
            ...
            .env("PICOCODEX_HTTP_PROXY_TEST_CHILD", "1")
            .env("PICOCODEX_HTTP_PROXY_TEST_ENDPOINT", endpoint)
        ...
        if let Some(status) = rejection_status {
            command.env(
                "PICOCODEX_HTTP_PROXY_TEST_EXPECT_REJECTION",
                status.to_string(),
            );
        }
    ...
    async fn proxy_connection_child() -> Result<()> {
        if env::var_os("PICOCODEX_HTTP_PROXY_TEST_CHILD").is_none() {
            return Ok(());
        }
        let endpoint = env::var("PICOCODEX_HTTP_PROXY_TEST_ENDPOINT")?;
        ...
        let expected_rejection = env::var("PICOCODEX_HTTP_PROXY_TEST_EXPECT_REJECTION")
            .ok()
