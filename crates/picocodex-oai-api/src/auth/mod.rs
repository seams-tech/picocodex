...
/// Authentication mode for the single `OpenAI` service family supported by Picocodex.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum OpenAiAuthMode {
    /// `OpenAI` Platform API key authentication.
    ApiKey,
    /// Authentication resolved entirely by the embedding host.
    HostManaged,
...
impl OpenAiAuthMode {
    ...
    pub const fn default_api_base_url(self) -> &'static str {
        match self {
            ...
            Self::HostManaged => "https://api.openai.com/v1",
    ...
    pub const fn default_websocket_url(self) -> &'static str {
        match self {
            ...
            Self::HostManaged => "wss://api.openai.com/v1/responses",
...
/// live in separate Picocodex crates. Applications should use [`OpenAiAuth`] constructors.
#[doc(hidden)]
...
impl OpenAiAuth {
    ...
    /// Creates authentication resolved entirely by the embedding host.
    ///
    /// The Rust runtime carries no bearer credential in this mode.
    #[must_use]
    pub fn host_managed() -> Self {
        Self {
            mode: OpenAiAuthMode::HostManaged,
            source: Arc::new(HostManagedAuth),
        }
    }
...
#[derive(Debug)]
struct HostManagedAuth;

impl OpenAiAuthSource for HostManagedAuth {
    fn validate(&self) -> Result<(), OpenAiAuthError> {
        Ok(())
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn host_managed_snapshots_contain_no_bearer() {
        let auth = OpenAiAuth::host_managed();
        let snapshot = auth.snapshot().await.unwrap();
        assert_eq!(snapshot.mode(), OpenAiAuthMode::HostManaged);
        assert_eq!(snapshot.bearer(), "");
    }
    ...
    fn auth_modes_select_their_service_endpoints() {
        ...
        assert_eq!(
            OpenAiAuthMode::HostManaged.default_websocket_url(),
            "wss://api.openai.com/v1/responses"
        );
    }
}
