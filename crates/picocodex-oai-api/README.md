# Picocodex `OpenAI` API

Tower-native building blocks for the `OpenAI` Responses API.

`picocodex-oai-api` is useful without the Picocodex agent loop. It owns the
...
# async fn run() -> Result<(), Box<dyn std::error::Error>> {
let auth_file = PathBuf::from(std::env::var("PICOCODEX_AUTH_FILE")?);
...
later runs. It uses Codex's `auth.json` format, so Codex and multiple Picocodex
...
use picocodex_oai_api::{OpenAi, ResponseEvent};
...
The higher-level `picocodex-agent` crate decides *when* to compact and how to
...
`picocodex-tools`. A standalone [`Session`] does not run a tool loop or attach
a `picocodex-tools::Tools` registry automatically. Use `picocodex-agent` for
...
- [`tools`] defines the shared tool contract; `picocodex-tools` supplies the
