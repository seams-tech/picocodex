...
fn retained_agent_event_trace(criterion: &mut Criterion) {
    let Some(path) = std::env::var_os("PICOCODEX_BENCH_EVENTS") else {
        eprintln!("PICOCODEX_BENCH_EVENTS is unset; skipping retained event-trace benchmarks");
        return;
    };
    ...
    let encoded =
        fs::read(&path).expect("PICOCODEX_BENCH_EVENTS should name a readable JSONL trace");
...
fn retained_inbound_events(path: &Path) -> Vec<Box<RawValue>> {
    let encoded = fs::read(path).expect("PICOCODEX_BENCH_EVENTS should name a readable trace");
