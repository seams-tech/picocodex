...
compile_error!(
    "picocodex-agent supports browser/JavaScript WebAssembly \
     (`wasm32-unknown-unknown`), not WASI targets"
);

extern crate self as picocodex_agent;
...
pub use agent::{
    AgentHandle, Picocodex, PicocodexBuilder, PromptRoute, Turn, TurnControl, TurnResult,
};
...
pub mod events {
    pub use picocodex_oai_api::events::{
    ...
    pub use picocodex_oai_api::responses::AgentMessageContent;
}
