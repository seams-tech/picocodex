...
impl ModelSessionState {
    pub(super) fn validate_workspace(&self, requested: Option<&str>) -> Result<()> {
        ...
        if requested != self.workspace {
            return Err(PicocodexError::WorkspaceChanged {
...
impl ConversationState {
    ...
    pub(super) fn new(history: Vec<ResponseItem>) -> Result<Self> {
        let canonical_context = history
            ...
            .cloned()
            .ok_or(PicocodexError::MalformedResponse {
    ...
    pub(super) fn resume(
        ...
        let managed = ManagedSessionState::resume(history)
            .map_err(|error| PicocodexError::InvalidSessionSnapshot(error.to_string()))?;
        Ok(Self {
            canonical_context: Arc::new(canonical_context),
            managed,
            continuation_policy: None,
    ...
    pub(super) fn prompt_history(&self) -> picocodex_oai_api::responses::ResponseHistory {
        self.managed.prompt_history()
    }

    pub(super) fn prompt_history_with_repair(
        &self,
    ) -> (picocodex_oai_api::responses::ResponseHistory, bool) {
        self.managed.prompt_history_with_repair()
    }

    pub(super) fn adopt_prompt_history(
        &mut self,
        history: picocodex_oai_api::responses::ResponseHistory,
    ) {
        self.managed.adopt_prompt_history(history);
    }

    pub(super) fn shared_history(&self) -> picocodex_oai_api::responses::ResponseHistory {
    ...
    pub(super) fn commit(&mut self) -> Result<()> {
        self.managed
            .commit()
            .map_err(|_| PicocodexError::MalformedResponse {
