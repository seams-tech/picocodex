...
/// idle. [`Picocodex::route_prompt`](crate::Picocodex::route_prompt) performs
...
impl Future for Turn {
    ...
    fn poll(mut self: Pin<&mut Self>, context: &mut Context<'_>) -> Poll<Self::Output> {
        Pin::new(&mut self.result)
            .poll(context)
            .map(|result| result.map_err(|_| PicocodexError::TurnStopped)?)
    }
}
...
pub(super) enum Command {
    ...
    Fork {
        ...
        result: oneshot::Sender<Result<(Picocodex, AgentEvents)>>,
    },
    Spawn {
        result: oneshot::Sender<Result<(Picocodex, AgentEvents)>>,
    },
