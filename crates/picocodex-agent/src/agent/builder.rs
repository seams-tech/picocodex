...
#[derive(Clone)]
pub struct PicocodexBuilder<F = StandardServiceFactory> {
...
impl<F> PicocodexBuilder<F> {
    ...
    /// [`Picocodex::set_thinking`] call affects subsequently accepted turns.
    #[must_use]
    ...
    ///
    /// Without this call the agent inherits the client default. A later
    /// [`Picocodex::set_fast_mode`] call affects subsequently accepted turns.
    #[must_use]
    ...
    /// lineage so [`Picocodex::fork_from`] can reject unrelated results.
    #[must_use]
    ...
    <F::Service as Service<ResponsesAttempt>>::Future: Send,
{
    ...
    pub fn build(self) -> Result<(Picocodex, AgentEvents)> {
        build(self)
    }
}
...
fn build<F>(builder: PicocodexBuilder<F>) -> Result<(Picocodex, AgentEvents)>
where
