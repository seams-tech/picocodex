...
impl AgentHandle {
    ...
    pub async fn spawn(&self) -> Result<(Picocodex, AgentEvents)> {
    ...
    pub async fn fork(&self) -> Result<(Picocodex, AgentEvents)> {
        let commands = self.commands()?;
        request_fork(&commands, None).await
    }

    fn commands(&self) -> Result<mpsc::Sender<Command>> {
        self.commands.upgrade().ok_or(PicocodexError::AgentStopped)
    }
}
...
impl Picocodex {
    ...
    #[must_use]
    pub fn builder<F>(openai: OpenAi<F>) -> PicocodexBuilder<F>
    where
    ...
    /// Returns the stable identity used by events and transport metadata.
    #[must_use]
    ...
    /// for model and tool cleanup. A returned `Ok(())` establishes a complete
    /// process boundary.
    ///
    ...
    pub async fn shutdown(&self) -> Result<()> {
        ...
        if initiate && self.commands.send(Command::Shutdown).await.is_err() {
            let outcome = match self.durability.shutdown().await {
                Ok(()) => Err(PicocodexError::AgentStopped),
                Err(error) => Err(error),
            };
        ...
        match receiver.await {
            Ok(Ok(())) => Ok(()),
            Ok(Err(error)) => Err(PicocodexError::Shutdown(error)),
            Err(_) => Err(PicocodexError::AgentStopped),
        }
    }
    ...
    pub async fn prompt(&self, prompt: impl Into<Prompt>) -> Result<Turn> {
        ...
        if prompt.instruction.is_empty() {
            return Err(PicocodexError::InvalidRequest(
        ...
        if self
            ...
            .is_err()
        {
            return Err(PicocodexError::AgentStopped);
        }
        Ok(Turn {
            control: TurnControl {
                key,
    ...
    pub async fn route_prompt(&self, prompt: impl Into<Prompt>) -> Result<PromptRoute> {
        ...
        match route_receiver
            .await
            .map_err(|_| PicocodexError::AgentStopped)??
        {
    ...
    ///
    /// ```
    /// # use picocodex_agent::{Picocodex, Result};
    /// # async fn compact_after_a_turn(agent: &Picocodex) -> Result<()> {
    /// agent
    ...
    /// same accepted-turn contract as prompt turns.
    ...
    pub async fn fork_from(&self, completed: &TurnResult) -> Result<(Self, AgentEvents)> {
        if completed.checkpoint.lineage_id() != self.lineage_id.as_ref() {
            return Err(PicocodexError::CheckpointLineageMismatch);
        }
...
async fn request_spawn(commands: &mpsc::Sender<Command>) -> Result<(Picocodex, AgentEvents)> {
...
pub(super) async fn request_command<T>(
    ...
    commands
        ...
        .await
        .map_err(|_| PicocodexError::AgentStopped)?;
    receiver.await.map_err(|_| PicocodexError::AgentStopped)?
}
