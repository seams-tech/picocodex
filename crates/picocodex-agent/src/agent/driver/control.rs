...
type SharedShutdownResult = std::result::Result<(), Arc<PicocodexError>>;
    ...
    while let Some(command) = commands.recv().await {
        match command {
            ...
            Command::RoutePrompt {
                ...
                drop(route_result.send(Err(PicocodexError::AgentStopped)));
                drop(turn_result);
            }
            Command::Fork { result, .. } | Command::Spawn { result } => {
                drop(result.send(Err(PicocodexError::AgentStopped)));
            }
    ...
    match command {
        Command::Fork { checkpoint, result } => {
            ...
            let outcome = checkpoint
                .ok_or(PicocodexError::ForkBeforeCompletedTurn)
                .and_then(|checkpoint| {
                    spawner.spawn_fork(
                        &checkpoint,
                        session_id,
        ...
        Command::Cancel { result, .. } => {
            drop(result.send(Err(PicocodexError::TurnNotCancellable)));
        }
