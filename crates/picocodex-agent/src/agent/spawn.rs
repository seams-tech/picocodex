    ...
    let (lineage_id, prompt_cache_key, initial_resume) = if let Some(snapshot) = resume {
            ...
            .is_some_and(|stored| stored != config.system_prompt())
        {
            return Err(PicocodexError::InvalidSessionSnapshot(
                "instructions do not match the resumed session".to_owned(),
            ));
        }
        if key
            .as_deref()
    ...
    let configured_workspace = workspace
        .map(|path| {
            path.into_os_string()
                .into_string()
                .map(Arc::<str>::from)
                .map_err(|path| PicocodexError::WorkspaceNotUtf8 {
...
pub(super) fn validate(config: &ModelConfig, prompt_cache_key: Option<&str>) -> Result<()> {
    config
        .auth
        .validate()
        .map_err(|error| PicocodexError::InvalidRequest(error.to_string()))?;
