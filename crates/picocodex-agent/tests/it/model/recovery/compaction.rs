...
#[tokio::test]
async fn manual_compaction_before_first_prompt_reinjects_cached_context() -> Result<()> {
...
async fn a_later_manual_compaction_replaces_a_stuck_manual_compaction() -> Result<()> {
    ...
    assert!(matches!(first.await?, Err(PicocodexError::TurnCancelled)));
    second.await??;
    assert!(matches!(
        agent.compact().await,
        Err(PicocodexError::Response(error))
