...
#[picocodex_tools::contract::async_trait]
impl picocodex_tools::Tool for NativeToolSearch {
    fn definition(&self) -> picocodex_tools::ToolDefinition {
        picocodex_tools::ToolDefinition::tool_search(
            "client",
            "Search caller-configured deferred tools.",
            picocodex_oai_api::responses::JsonSchema::from(json!({
                "type": "object",
                "properties": {
                    "query": {
                        "type": "string",
    ...
    async fn execute(
        &self,
        input: picocodex_tools::ToolInput,
        _context: picocodex_tools::ToolContext<'_>,
    ) -> picocodex_tools::ToolResult {
        let arguments: Value = input.decode_json()?;
        assert_eq!(arguments, json!({"query": "calendar create", "limit": 1}));
        Ok(picocodex_tools::ToolOutput::json(&json!([{
            "type": "function",
...
async fn mcp_tool_search_exposes_and_dispatches_a_native_namespace() -> Result<()> {
    ...
    let fixture = Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../picocodex-tools/tests/fixtures/mcp-stdio-server.mjs");
    let mcp = picocodex_tools::mcp::Mcp::builder()
        .server(
            "fixture",
            picocodex_tools::mcp::McpServer::stdio("node").arg(fixture.to_string_lossy()),
        )
        .build()?;
