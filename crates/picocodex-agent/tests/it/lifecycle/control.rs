...
async fn forking_before_a_completed_turn_is_typed() {
    let (agent, events) = Picocodex::builder(test_openai()).build().unwrap();
    let Err(error) = agent.fork().await else {
        panic!("fork unexpectedly succeeded");
    };
    assert!(matches!(error, PicocodexError::ForkBeforeCompletedTurn));
    drop((agent, events));
}
...
async fn steering_without_an_active_turn_is_typed() {
    let openai = OpenAi::builder("test")
        ...
        .build()
        .unwrap();
    let (agent, events) = Picocodex::builder(openai).build().unwrap();
    ...
    assert!(matches!(error, PicocodexError::TurnNotSteerable));
    drop((agent, events));
}
...
async fn turn_result_does_not_wait_for_attempt_event_producers_to_close() {
    ...
    assert!(matches!(
        ...
        picocodex_agent::transport::ResponsesAttemptKind::Generation
    ));
    ...
    loop {
        ...
        match event.kind {
            picocodex_agent::events::AgentEventKind::RunStarted => observed_start = true,
...
async fn an_agent_handle_does_not_keep_its_driver_alive() {
    let (handles, mut received_handles) = mpsc::unbounded_channel();
    let (agent, events) = Picocodex::builder(test_openai())
        .tools_factory(move |handle| {
            drop(handles.send(handle));
            Tools::builder().without_defaults().build()
        })
    ...
    assert!(matches!(error, PicocodexError::AgentStopped));
...
fn building_requires_a_tokio_runtime() {
    assert!(matches!(
        Picocodex::builder(test_openai()).build(),
        Err(PicocodexError::TokioRuntimeUnavailable)
    ));
}
