...
use picocodex_agent::{
    OpenAi, Picocodex, PicocodexError, ResponseError, Tools,
...
fn steering_content_is_traced_in_prompt_order() {
    ...
    const STEER: &str = "PICOCODEX_TRACE_STEER_SENTINEL";
    ...
    tracing::dispatcher::with_default(&dispatch, || {
        runtime.block_on(async {
            ...
            let openai = OpenAi::builder("test")
                ...
                .build()
                .unwrap();
            let (root, root_events) = Picocodex::builder(openai)
            ...
            let openai = OpenAi::builder("test")
                ...
                .build()
                .unwrap();
            let (plain, plain_events) = Picocodex::builder(openai).build().unwrap();
