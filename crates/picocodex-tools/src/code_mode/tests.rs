...
use picocodex_oai_api::{responses::ResponseItem, tools::ToolDefinition};
...
async fn execution_globals_do_not_leak_across_quickjs_contexts() -> Result<()> {
    ...
    let source = r"
const previous = globalThis.__picocodexContextGeneration;
globalThis.__picocodexContextGeneration = (previous || 0) + 1;
text({ previous: previous ?? null, current: globalThis.__picocodexContextGeneration });
";
...
async fn execution_prototype_mutations_do_not_leak_across_quickjs_contexts() -> Result<()> {
    ...
    let first = tools
        .execute_code(
            r#"
Object.prototype.__picocodexPoisoned = "yes";
text(({}).__picocodexPoisoned);
"#,
            test_context(&history),
        )
        .await;
    let second = tools
        .execute_code(
            r#"text(({}).__picocodexPoisoned ?? "clean");"#,
            test_context(&history),
        )
        .await;
...
text({
  ...
  hostCallback: typeof __picocodexTool,
...
fn test_tools(workspace: &std::path::Path) -> ToolRuntime {
    ToolRuntime::new(
        ...
        Some(WebSearchConfig {
            ...
            auth: picocodex_oai_api::auth::OpenAiAuth::api_key("test-key"),
        }),
