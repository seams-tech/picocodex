//! Background-handshaken MCP tools for Picocodex Code Mode.
...
use picocodex_oai_api::tools::ToolDefinition;
...
mod tests {
    ...
    async fn dropping_mcp_terminates_stdio_descendants() {
        ...
        let directory = std::env::temp_dir().join(format!(
            "picocodex-mcp-descendant-{}-{nonce}",
            std::process::id()
        ));
        ...
        let mcp = Mcp::builder()
            .server(
                ...
                McpServer::stdio("node")
                    ...
                    .env("PICOCODEX_MCP_DESCENDANT_MARKER", marker.to_string_lossy()),
            )
            .build()
            .unwrap();
        mcp.start();
    ...
    async fn smoke_http_servers_from_environment() {
        let configured = std::env::var("PICOCODEX_MCP_SMOKE_SERVERS")
            .expect("set PICOCODEX_MCP_SMOKE_SERVERS to comma-separated NAME=URL entries");
        let bearers = std::env::var("PICOCODEX_MCP_SMOKE_BEARERS")
            .ok()
            .map(|configured| {
                configured
                    .split(',')
