...
const MCP_USER_AGENT: &str = concat!("picocodex-mcp-client/", env!("CARGO_PKG_VERSION"));
...
mod tests {
        ...
        S: Subscriber + for<'lookup> LookupSpan<'lookup>,
    {
        fn on_event(&self, event: &Event<'_>, _context: Context<'_, S>) {
            if event.metadata().target() == "picocodex_tools" {
    ...
    fn streamable_http_headers_have_an_overridable_default_user_agent() {
        ...
        let expected = concat!("picocodex-mcp-client/", env!("CARGO_PKG_VERSION"));
        assert_eq!(
            resolved
                .get(&USER_AGENT)
