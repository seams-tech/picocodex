...
impl HostedToolRuntime {
    ...
    pub(crate) fn model_contract(
        ...
        let direct_enabled = matches!(mode, HostedToolMode::Direct | HostedToolMode::Hybrid);
        if direct_enabled {
        ...
        } else if let Ok(mut names) = self.direct_tool_names.write() {
            names.clear();
        }
        ...
        let exec = ToolDefinition::custom(
            "exec",
            description,
            CustomToolFormat::grammar("lark", EXEC_GRAMMAR),
        );
        if mode == HostedToolMode::Hybrid {
            definitions.push(exec);
            (definitions, code_mode_tool_names)
        } else {
            (vec![exec], code_mode_tool_names)
        }
    }
    ...
    pub fn contains(&self, name: &str) -> bool {
        self.host.as_ref().is_some_and(|host| {
            matches!(
                host.tool_mode(),
                HostedToolMode::Direct | HostedToolMode::Hybrid
            ) && self
                .direct_tool_names
                .read()
...
mod tests {
    use picocodex_oai_api::tools::ToolOutputBody;
    use serde_json::json;
