...
mod tests {
    ...
    async fn child_process_receives_explicit_environment_overrides() {
        let sessions = ShellSessions::with_environment(Arc::new(vec![
            (
                OsString::from("PICOCODEX_EGRESS_TEST"),
                OsString::from("injected"),
            ),
            (
                OsString::from("HTTPS_PROXY"),
                OsString::from("http://picocodex:proxy-secret-value@127.0.0.1:1234"),
            ),
            (
                OsString::from("PICOCODEX_EGRESS_PROXY_PASSWORD"),
        ...
        let result = sessions
            .execute(
                ExecCommand::new(
                    "printf '%s|%s' \"$PICOCODEX_EGRESS_TEST\" \"$HTTPS_PROXY\"".to_owned(),
                    None,
        ...
        assert_eq!(
            result.output,
            "injected|http://picocodex:[REDACTED]@127.0.0.1:1234"
        );
    }
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        let directory = std::env::temp_dir().join(format!(
            "picocodex-background-process-{}-{nonce}",
            std::process::id()
        ));
    ...
    async fn cancellation_terminates_shell_descendants() -> Result<(), Box<dyn std::error::Error>> {
        let nonce = SystemTime::now()
            ...
            .as_nanos();
        let directory = std::env::temp_dir().join(format!(
            "picocodex-cancelled-process-{}-{nonce}",
            std::process::id()
        ));
        std::fs::create_dir_all(&directory)?;
    ...
    async fn tty_command_has_a_terminal_and_accepts_stdin() {
        let ready =
            std::env::temp_dir().join(format!("picocodex-pty-ready-{}", std::process::id()));
