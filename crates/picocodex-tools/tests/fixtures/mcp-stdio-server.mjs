...
const toolCount = Number.parseInt(
  process.env.PICOCODEX_MCP_FIXTURE_TOOL_COUNT ?? "1",
  10,
);
if (process.env.PICOCODEX_MCP_DESCENDANT_MARKER) {
  spawn(process.execPath, [
    "-e",
    "setTimeout(() => require('node:fs').writeFileSync(process.env.PICOCODEX_MCP_DESCENDANT_MARKER, 'survived'), 750)",
  ], {
    env: process.env,
    stdio: "ignore",
  });
  ...
  if (request.method === "initialize") {
    send({
      ...
      result: {
        ...
        capabilities: { tools: {} },
        serverInfo: { name: "picocodex-test-mcp", version: "0.1.0" },
      },
    });
