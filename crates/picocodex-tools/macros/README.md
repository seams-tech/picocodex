# Picocodex Tools Macros

Procedural macro implementation for `picocodex-tools`.

Applications should not add this package directly. Add `picocodex-tools` or
the `picocodex` facade and import the re-exported `tool` macro:

```rust,ignore
use picocodex_tools::{Tools, tool};
...
boundary. The supported user-facing paths are `picocodex_tools::tool` and
`picocodex::tool`.
