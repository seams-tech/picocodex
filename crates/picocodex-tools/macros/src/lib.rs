...
fn expand_tool(
    ...
    let handler_ident = format_ident!("__picocodex_tool_{original_ident}");
    let input_ident = format_ident!("__PicocodexTool{}Input", pascal_case(original_ident));
    ...
    let (picocodex, picocodex_path) = picocodex_path();
    let serde_path = LitStr::new(
        &format!("{picocodex_path}::__private::serde"),
        Span::call_site(),
    );
    let schemars_path = LitStr::new(
        &format!("{picocodex_path}::__private::schemars"),
        Span::call_site(),
    );
    ...
    Ok(quote! {
        ...
        #[derive(
            #picocodex::__private::serde::Deserialize,
            #picocodex::__private::schemars::JsonSchema
        )]
        #[serde(
            crate = #serde_path,
            deny_unknown_fields
        ...
        #[#picocodex::__private::async_trait]
        impl #picocodex::__private::Tool for #original_ident {
            fn definition(&self) -> #picocodex::__private::ToolDefinition {
                #picocodex::__private::ToolDefinition::function(
                    #tool_name,
                    #description,
                    #picocodex::__private::schema_for::<#input_ident>(),
                )
                .with_output_schema(#picocodex::__private::schema_for::<#output>())
            }
            ...
            async fn execute(
                &self,
                input: #picocodex::__private::ToolInput,
                _context: #picocodex::__private::ToolContext<'_>,
            ) -> #picocodex::__private::ToolResult {
                let input = input.decode_json::<#input_ident>()?;
                match #handler_ident(#(#calls),*).await {
                    Ok(output) => Ok(#picocodex::__private::ToolOutput::json(&output)),
                    Err(error) => Err(
...
fn picocodex_path() -> (proc_macro2::TokenStream, String) {
    for package in ["picocodex", "picocodex-agent", "picocodex-tools"] {
    ...
    (quote!(::picocodex_tools), "::picocodex_tools".to_owned())
}
