...
`picocodex-tools` is useful without the Picocodex agent loop. It provides the
...
The shared contract types are defined by `picocodex-oai-api` and re-exported
...
# fn build() -> Result<(), picocodex_tools::ToolsBuildError> {
...
such as `picocodex-vm-guest`. With default features disabled, it exposes the
