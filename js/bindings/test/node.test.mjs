...
test("Node-hosted WASM preserves follow-ons, cache identity, events, and custom tools", async () => {
  ...
  const scenario = (async () => {
    ...
    assert.equal(socket.request.headers["user-agent"], `picocodex-wasm/${PACKAGE_VERSION}`);
...
test("Node can load an application-owned web module and resume a session snapshot", async () => {
  const server = await startServer();
  const wasm = await readFile(new URL("../pkg-web/picocodex_bg.wasm", import.meta.url));
  ...
  const snapshot = {
    version: 1,
    model: "gpt-5.6-sol",
    lineage_id: "actor-checkpoint-lineage",
    prompt_cache_key: "actor-checkpoint-lineage",
  ...
  const scenario = (async () => {
    ...
    sendFinal(socket, "resp-snapshot-resumed", "amber");
  })();
...
test("independent agents keep their host connections isolated", async () => {
  ...
  const leftTools = globalThis.picocodexHost.toolDefinitions(SESSION_IDS.left);
  const rightTools = globalThis.picocodexHost.toolDefinitions(SESSION_IDS.right);
