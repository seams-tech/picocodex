...
#[wasm_bindgen]
extern "C" {
    #[wasm_bindgen(js_namespace = ["globalThis", "picocodexHost"], js_name = emitEvent)]
    fn host_emit_event(event: &str);

    #[wasm_bindgen(catch, js_namespace = ["globalThis", "picocodexHost"], js_name = executeCode)]
    fn host_execute_code(source: &str, session_id: &str, call_id: &str)
    -> Result<Promise, JsValue>;

    #[wasm_bindgen(catch, js_namespace = ["globalThis", "picocodexHost"], js_name = executeTool)]
    ...
    #[wasm_bindgen(js_namespace = ["globalThis", "picocodexHost"], js_name = toolMode)]
    fn host_tool_mode(session_id: &str) -> String;

    #[wasm_bindgen(catch, js_namespace = ["globalThis", "picocodexHost"], js_name = toolDefinitions)]
...
impl JavaScriptCodeModeHost {
    fn new() -> Self {
        Self {
            mode: match host_tool_mode("").as_str() {
                "direct" => HostedToolMode::Direct,
                "hybrid" => HostedToolMode::Hybrid,
                _ => HostedToolMode::Code,
            },
        }
    }
}
...
#[wasm_bindgen(js_name = Picocodex)]
pub struct WasmPicocodex {
    inner: RustPicocodex,
}

#[wasm_bindgen(js_class = Picocodex)]
impl WasmPicocodex {
    ...
    pub fn new(config_json: &str) -> Result<Self, JsValue> {
        ...
        let openai = OpenAi::builder(auth(&config.auth))
            ...
            .map_err(js_error)?;
        let mut builder =
            RustPicocodex::builder(openai).tools(HostedTools::new(JavaScriptCodeModeHost::new()));
...
impl WasmPicocodex {
    fn from_parts(inner: RustPicocodex, events: AgentEvents) -> Self {
...
impl WasmTurn {
    fn accept(agent: RustPicocodex, prompt: Prompt) -> Self {
...
fn validate(config: &WasmConfig) -> Result<(), JsValue> {
    if let WasmAuthConfig::ApiKey { api_key } = &config.auth
...
fn auth(config: &WasmAuthConfig) -> OpenAiAuth {
    match config {
        WasmAuthConfig::ApiKey { api_key } => OpenAiAuth::api_key(api_key.clone()),
        WasmAuthConfig::HostManaged => OpenAiAuth::host_managed(),
    }
}
