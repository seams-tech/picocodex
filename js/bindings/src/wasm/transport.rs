...
use js_sys::Promise;
use picocodex::oai::transport::host::{
...
#[wasm_bindgen]
extern "C" {
    #[wasm_bindgen(catch, js_namespace = ["globalThis", "picocodexHost"], js_name = connect)]
    fn host_connect(
        endpoint: &str,
        bearer_token: &str,
    ...
    #[wasm_bindgen(catch, js_namespace = ["globalThis", "picocodexHost"], js_name = send)]
    fn host_send(handle: u32, message: &str) -> Result<Promise, JsValue>;

    #[wasm_bindgen(catch, js_namespace = ["globalThis", "picocodexHost"], js_name = next)]
    fn host_next(handle: u32, timeout_ms: u32) -> Result<Promise, JsValue>;

    #[wasm_bindgen(js_namespace = ["globalThis", "picocodexHost"], js_name = close)]
    fn host_close(handle: u32);

    #[wasm_bindgen(catch, js_namespace = ["globalThis", "picocodexHost"], js_name = sleep)]
