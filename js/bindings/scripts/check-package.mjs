...
const requiredFiles = [
  ...
  "node/index.d.mts",
  "wasm.d.mts",
  "pkg-web/picocodex.js",
  "pkg-web/picocodex.d.ts",
  "pkg-web/picocodex_bg.wasm",
  "pkg-node/picocodex.js",
  "pkg-node/picocodex.d.ts",
  "pkg-node/picocodex_bg.wasm",
];
...
export async function checkPackage(packageRoot = root) {
  ...
  assert.equal(packageJson.name, "picocodex");
  ...
  assert.equal(packageJson.exports?.["./wasm"]?.import, "./pkg-web/picocodex_bg.wasm");
  ...
  for (const target of ["web", "node"]) {
    const wasm = await readFile(
      new URL(`pkg-${target}/picocodex_bg.wasm`, packageRoot),
    );
  ...
  console.log(`picocodex@${packageJson.version} package artifacts are complete`);
}
