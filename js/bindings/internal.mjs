...
export function defineRuntime(definition) {
  if (!definition || typeof definition.create !== "function") {
    throw new TypeError("a Picocodex runtime must define create(options)");
  }
  return Object.freeze({
    key: definition.key ?? "custom",
    name: definition.name ?? "Picocodex Agent",
...
export async function createAgentClient(runtime, options = {}) {
  if (!runtime || typeof runtime.create !== "function") {
    throw new TypeError("createAgent requires a Picocodex runtime");
  }
...
export async function shutdown(agent) {
  ...
  if (state.disposed) throw new Error("the Picocodex agent has been disposed");
  if (typeof state.raw.shutdown !== "function") {
    throw new Error("this Picocodex runtime does not expose graceful shutdown");
  }
  state.disposed = true;
...
export function subscribeAgentEvents(agent, listener, options = {}, onRelease) {
  ...
  if (typeof state.runtime.subscribe !== "function") {
    throw new Error("this Picocodex runtime does not expose agent events");
  }
...
export function toWasmConfig(options = {}) {
  ...
  if (!auth || (auth.kind !== "api_key" && auth.kind !== "host_managed")) {
    throw new TypeError("auth must select api_key or host_managed");
  }
  if (auth.kind === "api_key"
    && (typeof auth.api_key !== "string" || !auth.api_key.trim())) {
...
export function activateHost(host) {
  if (!host || typeof host.connect !== "function") {
    throw new TypeError("a Picocodex host must define connect()");
  }
  activeHost = host;
  globalThis.picocodexHost = hostBridge;
}

export function bindHostSession(host, sessionId) {
  const existing = hostSessions.get(sessionId);
  if (existing && existing !== host) {
    throw new Error(`Picocodex session ID is already active: ${sessionId}`);
  }
...
const hostBridge = Object.freeze({
  ...
  sleep(sessionId, milliseconds) {
    ...
    if (typeof host.sleep !== "function") {
      throw new TypeError("the selected Picocodex host must define sleep(milliseconds)");
    }
...
function createAgent(raw, runtime) {
  if (!raw || typeof raw.prompt !== "function") {
    throw new TypeError("the runtime returned an invalid Picocodex agent handle");
  }
  const state = {
    raw,
    runtime,
...
function requiredSessionHost(sessionId) {
  ...
  if (!host) throw new Error(`no Picocodex host is active for session: ${sessionId}`);
  return host;
}
...
async function joinAgentShutdown(state) {
  ...
  if (shutdownFailed && cleanupFailed) {
    ...
    throw new AggregateError(
      ...
      "Picocodex driver shutdown and resource release both failed",
    );
  }
...
function requiredActiveHost() {
  if (!activeHost) throw new Error("no Picocodex host is active");
  return activeHost;
}
...
function createTurn(raw, agent) {
  if (!raw || typeof raw.result !== "function") {
    throw new TypeError("the runtime returned an invalid Picocodex turn handle");
  }
...
function createTurnResult(raw) {
  if (
    ...
    raw?.free?.();
    throw new TypeError("the runtime returned an invalid Picocodex turn result");
  }
  const state = {
    raw,
...
function knownAgentState(agent) {
  ...
  if (!state) throw new TypeError("expected a Picocodex agent");
  return state;
}

function turnState(turn) {
  const state = turnStates.get(turn);
  if (!state) throw new TypeError("expected a Picocodex turn");
  if (state.disposed) throw new Error("the Picocodex turn has been disposed");
  return state;
}

function resultState(result) {
  const state = resultStates.get(result);
  if (!state) throw new TypeError("expected a completed Picocodex turn result");
  return state;
}
...
function throwCleanupErrors(errors) {
  ...
  if (errors.length > 1) {
    throw new AggregateError(errors, "multiple Picocodex resources failed to release");
  }
}
